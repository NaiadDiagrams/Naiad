...
/// <see href="https://github.com/mono/SkiaSharp">SkiaSharp</see>. The parse, layout and styling that
/// produce the diagram are exactly the same code path as <see cref="Mermaid.Render(string, RenderOptions)"/>;
/// this only swaps the final stage from emitting SVG to rasterizing it with Skia. The companion
/// Naiad.ImageSharp package renders the same way through ImageSharp. Rasterization is controlled by
/// <see cref="RenderOptions.Png"/> (scale and background).
/// </summary>
public static class SkiaRenderer
{
    ...
    // the Skia rasterize + encode path on its own, without the parse/layout cost dominating the number.
    ...
    static void RenderDocument(SvgDocument document, Stream stream, RenderOptions options)
    {
        ...
        using var surface = SvgRasterizer.Paint(document, options.Png.Scale, (width, height) => new SkiaSurface(width, height, background, options.Png.Compression));

...
public class PositionTests
{
    ...
    public async Task RespectsRanksep()
    {
        ...
        g.SetEdge("a", "b");
        Position.Run(g);
        await Assert.That(g.NodeLabel("b").Y!.Value).IsEqualTo(100 + 1000 + 80 / 2.0);
    }
    ...
    public async Task UseTheLargestHeightInEachRankWithRanksep()
    {
        ...
        g.SetEdge("a", "c");
        Position.Run(g);
        await Assert.That(g.NodeLabel("a").Y!.Value).IsEqualTo(100 / 2.0);
        await Assert.That(g.NodeLabel("b").Y!.Value).IsEqualTo(100 / 2.0); // Note we used 100 and not 80 here
        await Assert.That(g.NodeLabel("c").Y!.Value).IsEqualTo(100 + 1000 + 90 / 2.0);
    }
    ...
    public async Task RespectsNodesep()
    {
        ...
        Position.Run(g);
        await Assert.That(g.NodeLabel("b").X!.Value).IsEqualTo(g.NodeLabel("a").X!.Value + 50 / 2.0 + 1000 + 70 / 2.0);
    }
    ...
    public async Task ShouldNotTryToPositionTheSubgraphNodeItself()
    {
        ...
        g.SetParent("a", "sg1");
        Position.Run(g);
        await Assert.That(g.NodeLabel("sg1").X).IsNull();
        await Assert.That(g.NodeLabel("sg1").Y).IsNull();
    }
    ...
    public async Task AlignsNodesToTopOfRankWhenRankalignIsTop()
    {
        ...
        Position.Run(g);
        await Assert.That(g.NodeLabel("a").Y!.Value).IsEqualTo(100 / 2.0);
        await Assert.That(g.NodeLabel("b").Y!.Value).IsEqualTo(60 / 2.0);
    }
    ...
    public async Task AlignsNodesToBottomOfRankWhenRankalignIsBottom()
    {
        ...
        g.SetNode("b", new() { Width = 50, Height = 60, Rank = 0, Order = 1 });
        Position.Run(g);
        await Assert.That(g.NodeLabel("a").Y!.Value).IsEqualTo(100 - 100 / 2.0);
        await Assert.That(g.NodeLabel("b").Y!.Value).IsEqualTo(100 - 60 / 2.0);
    }
    ...
    public async Task AlignsNodesToCenterOfRankWhenRankalignIsCenter()
    {
        ...
        Position.Run(g);
        await Assert.That(g.NodeLabel("a").Y!.Value).IsEqualTo(100 / 2.0);
        await Assert.That(g.NodeLabel("b").Y!.Value).IsEqualTo(100 / 2.0);
    }
}

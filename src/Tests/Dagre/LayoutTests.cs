public class LayoutTests
{
    ...
    public async Task CanLayoutAnEdgeWithALabel()
    {
        ...
        graph.SetNode("a", new() { Width = 50, Height = 100 });
        graph.SetNode("b", new() { Width = 75, Height = 200 });
        graph.SetEdge("a", "b", new() { Width = 60, Height = 70, Labelpos = LabelPosition.Center });
        Layout.Run(graph);
    ...
    public async Task CanLayoutAnEdgeWithALongLabel(Direction rankdir)
    {
        ...
        graph.SetEdge("a", "c", new() { Width = 2000, Height = 10, Labelpos = LabelPosition.Center });
    ...
    public async Task CanApplyAnOffset(Direction rankdir)
    {
        ...
        graph.SetEdge("a", "b", new() { Width = 10, Height = 10, Labelpos = LabelPosition.Left, Labeloffset = 1000 });
        graph.SetEdge("c", "d", new() { Width = 10, Height = 10, Labelpos = LabelPosition.Right, Labeloffset = 1000 });
        Layout.Run(graph);
    ...
    public async Task CanLayoutALongEdgeWithALabel()
    {
        ...
        graph.SetNode("a", new() { Width = 50, Height = 100 });
        graph.SetNode("b", new() { Width = 75, Height = 200 });
        graph.SetEdge("a", "b", new() { Width = 60, Height = 70, Minlen = 2, Labelpos = LabelPosition.Center });
        Layout.Run(graph);
    ...
    public async Task BoundingBoxEdgeLabelposL(Direction rankdir)
    {
        ...
        graph.SetNode("a", new() { Width = 100, Height = 100 });
        graph.SetNode("b", new() { Width = 100, Height = 100 });
        graph.SetEdge("a", "b", new() { Width = 1000, Height = 2000, Labelpos = LabelPosition.Left, Labeloffset = 0 });
        Layout.Run(graph);
        if (rankdir is Direction.TopToBottom or Direction.BottomToTop)
        {
